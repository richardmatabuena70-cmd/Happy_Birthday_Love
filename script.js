...
I love you endlessly, my love. Thank you for making my heart feel at home. ❤️`;
  ...
  function applyDefaults() {
    ...
    DEFAULT_PICTURES.forEach((item, idx) => {
      const isVideo = typeof item === 'string' && item.match(/\.(mp4|webm|ogg|mov)$/i);
      const caption = COMPLIMENTS_GIRL[idx % COMPLIMENTS_GIRL.length];
      if (Array.isArray(item)) addPictureCard(item[0], item[1] || caption, isVideo);
      else addPictureCard(item, caption, isVideo);
    });
  }
...
These balloons are like my love for you—light, colorful, and always rising no matter what. May they lift your spirits and remind you of how high my love for you soars. Happy birthday, my love. I hope this day is filled with joy, Laughter, and all the Happiness you deserve. I can't wait to celebrate many more birthdays with you, when I able to go there to meet you so let this balloons carry my love for you iloceyousomuch mahal ko.`
  ...
  imageInput.addEventListener('change', (e) => {
    Array.from(e.target.files).forEach(file => {
      ...
      r.onload = (ev) => addPictureCard(ev.target.result, file.name.replace(/\.[^.]+$/, ''), isVideo);
      r.readAsDataURL(file);
    });
    imageInput.value = '';
  });
  function addPictureCard(src, caption, isVideo) {
    const tilt = (Math.random() * 6 - 3).toFixed(1);
    const card = document.createElement('div');
    card.className = 'picture-card';
    card.style.setProperty('--tilt', tilt + 'deg');
    const media = isVideo
      ? `<video src="${src}" alt="Memory" playsinline loop muted onplay="this.muted=false"></video>`
      : `<img src="${src}" alt="Memory" />`;
    card.innerHTML = `
      <div class="photo-inner">${media}</div>
    ...
    card.querySelector('.photo-inner').addEventListener('click', () => openLightbox(card));
  ...
  picturesGrid.addEventListener('drop', (e) => {
    ...
    Array.from(e.dataTransfer.files).filter(f=>f.type.startsWith('image/') || f.type.startsWith('video/')).forEach(file => {
      const isVideo = file.type.startsWith('video/');
      const r = new FileReader(); r.onload = (ev) => addPictureCard(ev.target.result, file.name.replace(/\.[^.]+$/, ''), isVideo); r.readAsDataURL(file);
    });
  });
  ...
  function openLightbox(card) {
    lightboxList = Array.from(picturesGrid.querySelectorAll('.picture-card'));
    lightboxIndex = lightboxList.indexOf(card);
  ...
  function renderLightbox(card) {
    ...
    const media = card.querySelector('.photo-inner img, .photo-inner video');
    const caption = card.querySelector('.polaroid-caption')?.textContent || 'our memory';
  ...
  function lightboxNav(d) {
    ...
    renderLightbox(lightboxList[lightboxIndex]);
  ...
  slideshowBtn.addEventListener('click', () => {
    ...
    if (cards.length === 0) { alert('Add some photos first 💕'); return; }
    ...
    openLightbox(cards[0]);
    let i = 0;
    slideTimer = setInterval(() => { i = (i+1) % cards.length; openLightbox(cards[i]); }, 2500);
  });
  ...
  dlBtn.addEventListener('click', () => {
    ...
    a.href = src;
    a.download = isVideo ? 'memory.mp4' : 'memory.png';
    a.click();
    blip(700,0.1,'sine',0.05);
  });
  ...
  addPictureCard = function(src, caption, isVideo) {
    _origAddPictureCard(src, caption, isVideo);
